...
class Openblas(CMakePackage, MakefilePackage):
    ...
    # 0.3.30/0.3.32/0.3.33/0.3.34; 0.3.31 not in recipe). Older releases refused.
    conflicts(
        "+force_avx512",
        when="@:0.3.29",
...
class MakefileBuilder(makefile.MakefileBuilder):
    ...
    def make_defs(self):
        ...
        if self.spec.satisfies("target=m1:"):
            ...
            if (
                not self.spec.satisfies("target=x86_64_v4:")
                and not self.spec.satisfies("+force_avx512")
            ):
